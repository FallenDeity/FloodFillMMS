...
class BFS:
    ...
    def run(self, debug: bool = False) -> typing.List[PointType]:
        """
        The run function is used to run the BFS algorithm. It creates a visited set and a queue. The queue is a deque
        that contains the current cell. We then loop through the queue until it is empty. We pop the first cell from
        the queue and add it to the visited set. We then check if the current cell is the destination cell. If it is,
        we return the path from the start cell to the destination cell. If it is not, we generate the children of the
        current cell and add them to the queue. We then loop through the children and add them to the queue if they
        have not been visited yet. We then repeat the process until we find the destination cell or the queue is empty.
